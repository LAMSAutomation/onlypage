...
import { 
  ...
  RefreshCw, Check, Code, MapPin, Smile, Globe, Scissors, Paperclip, CheckSquare, Eye, X, Loader2, UploadCloud, QrCode, Download, Lock
} from 'lucide-react';
...
import { PRICING_PLANS, ADDONS } from '../src/data';
...
import { getUserSubscription, getTrialInfo, getPlanInvoiceAmount } from '@/lib/subscriptions';
import { getSiteUsage, SiteUsage } from '@/lib/usage';
import WhatsAppBroadcast from './ui/whatsapp-broadcast';
import { ReferralProgram } from './ui/referral-program';
import { EmailDrip } from './ui/email-drip';
...
export function Dashboard({ activeTab, setActiveTab, dashboardMode, site }: DashboardProps) {
  ...
  const [gmailConnected, setGmailConnected] = useState(() => {
    return localStorage.getItem(`onlypage_gmail_connected_${site.id}`) === 'true';
  });
  const [showGmailModal, setShowGmailModal] = useState(false);
  const [connectingGmail, setConnectingGmail] = useState(false);
  const [gmailEmail, setGmailEmail] = useState(() => {
    return localStorage.getItem(`onlypage_gmail_email_${site.id}`) || `${site.subdomain || 'business'}@gmail.com`;
  });
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (data.user?.email) {
        setGmailEmail(data.user.email);
        const isGoogleUser = data.user.app_metadata?.provider === 'google' || data.user.identities?.some(i => i.provider === 'google');
        if (isGoogleUser) {
          setGmailConnected(true);
          localStorage.setItem(`onlypage_gmail_connected_${site.id}`, 'true');
          localStorage.setItem(`onlypage_gmail_email_${site.id}`, data.user.email);
        }
      }
    });
  }, [site.id]);
  ...
  // QR Code Modal State
  const [qrModalData, setQrModalData] = useState<{ isOpen: boolean; title: string; url: string }>({
    isOpen: false,
    title: '',
    url: ''
  });
  ...
  const getDynamicVisitorsData = () => {
    ...
    // Baseline view metrics if nothing recorded
    if (pageViews.length === 0 && contacts.length === 0) {
      return [
        { day: 'Mon', Visitors: 0, Conversion: 0 },
        { day: 'Tue', Visitors: 0, Conversion: 0 },
        { day: 'Wed', Visitors: 0, Conversion: 0 },
        { day: 'Thu', Visitors: 0, Conversion: 0 },
        { day: 'Fri', Visitors: 0, Conversion: 0 },
        { day: 'Sat', Visitors: 0, Conversion: 0 },
        { day: 'Sun', Visitors: 0, Conversion: 0 }
      ];
    }
    return dataMap;
  };
  ...
  // 11. SEO Manager - Dynamic state & calculations
  const [seoSubTab, setSeoSubTab] = useState<'overview' | 'meta' | 'keywords' | 'technical'>('overview');
  const [seoMetaTitle, setSeoMetaTitle] = useState(() => site.theme?.seoTitle || `${site.business_name} | Official Website`);
  const [seoMetaDesc, setSeoMetaDesc] = useState(() => site.theme?.seoDesc || `Discover premium products, services, and online bookings at ${site.business_name}. Order online or book appointments.`);
  const [seoKeywordsStr, setSeoKeywordsStr] = useState(() => site.theme?.seoKeywords || `${site.business_name.toLowerCase()}, online store, buy online, appointments, services`);
  const [fixingSeo, setFixingSeo] = useState(false);
  const [generatingKeywords, setGeneratingKeywords] = useState(false);
  const [keywordList, setKeywordList] = useState<any[]>([
    { word: `${site.business_name.toLowerCase()} near me`, intent: 'Local Intent 📍', difficulty: 'Low', volume: 'High 📈' },
    { word: `buy ${site.business_name.toLowerCase()} products online`, intent: 'Transactional 💳', difficulty: 'Medium', volume: 'High 📈' },
    { word: `best ${site.business_name.toLowerCase()} services`, intent: 'High Commercial 💎', difficulty: 'Low', volume: 'Medium 📊' },
    { word: `book ${site.business_name.toLowerCase()} appointment`, intent: 'Actionable 🚀', difficulty: 'Low', volume: 'High 📈' }
  ]);
  ...
  // 19b. Subscription plan gating state (loaded from Supabase)
  const [userPlanId, setUserPlanId] = useState<string>('free');
  const [realUsage, setRealUsage] = useState<SiteUsage | null>(null);
  const [planLoading, setPlanLoading] = useState(true);
  const [showUpgradeModal, setShowUpgradeModal] = useState(false);
  const [upgradeFeature, setUpgradeFeature] = useState('');

  // 19c. Trial period state
  const [trialDaysLeft, setTrialDaysLeft] = useState(0);
  const [isTrialing, setIsTrialing] = useState(false);
  const [trialExpired, setTrialExpired] = useState(false);

  // 20. Email notification state (trial ending, payment success, upgrade/downgrade)
  ...
  const [savingEmailNotifs, setSavingEmailNotifs] = useState(false);

  // Load real subscription + trial info on mount
  useEffect(() => {
    if (!site?.id) return;
    (async () => {
      try {
        ...
        if (sub) {
          setUserPlanId(sub.plan_id);
          setBillingPlan(sub.plan_id === 'free' ? 'Free Plan' : sub.plan_id === 'starter' ? 'Starter Plan' : 'Business Plan');
        }
      } catch (e) {
    ...
    (async () => {
      try {
        ...
        setTrialDaysLeft(trial.daysRemaining);
        setTrialExpired(trial.isExpired);
      } catch (e) {
        console.error('Failed to load trial info', e);
      }
    })();
    ...
    (async () => {
      try {
        const usage = await getSiteUsage(site.id);
        setRealUsage(usage);
      } catch (e) {
  ...
  const handleConfirmUpgrade = async (planId: string) => {
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({ user_id: user.id, plan_id: planId, status: 'active' }, { onConflict: 'user_id' });
      if (error) throw error;
      setUserPlanId(planId);
      const planName = planId === 'free' ? 'Free Plan' : planId === 'starter' ? 'Starter Plan' : 'Business Plan';
      setBillingPlan(planName);
      setShowUpgradeModal(false);
      showToast(`Switched to ${planName}!`);
    } catch (err: any) {
      showToast('Error updating plan: ' + (err.message || 'Please try again'));
    }
  };
  ...
  const getDynamicInvoices = () => {
    ...
    while (tempDate >= createdDate && count < 3) {
      ...
      const planAmount = getPlanInvoiceAmount(userPlanId);
      invoices.push({
    ...
    if (invoices.length === 0) {
      ...
      invoices.push({
        ...
        amount: getPlanInvoiceAmount(userPlanId),
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* DYNAMIC DASHBOARD SWITCHER HEADER BAR (LANDING PAGE BRANDING) */}
      <div className="bg-[#18201d] rounded-3xl p-6 sm:p-8 text-white relative overflow-hidden shadow-[0_20px_50px_rgba(24,32,29,0.15)] border border-[#18201d]/10 select-none">
        {/* Backdrop visual ambient glow overlay matching Landing Page */}
        <div className="absolute -top-10 -right-10 w-80 h-80 bg-lime-300/15 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute -bottom-10 -left-10 w-60 h-60 bg-[#70a10d]/10 rounded-full blur-2xl pointer-events-none" />

        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#e3f0bb] rounded-full text-[10px] font-black tracking-wide uppercase text-[#4c7005] border border-[#70a10d]/20">
              <Sparkles size={11} className="text-[#70a10d]" />
              <span>Workspace Active: {dashboardMode.toUpperCase()}</span>
            </div>
            <h1 className="text-3xl font-black tracking-[-0.04em] leading-tight text-white">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            <button
              ...
              className="px-4 py-2.5 bg-white hover:bg-[#e3f0bb] text-[#18201d] text-xs font-black rounded-xl transition-all cursor-pointer shadow-sm flex items-center gap-1.5 hover:-translate-y-0.5"
            >
              <Code size={14} className="text-[#18201d]" />
              <span>Edit Live Site</span>
            </button>
            
            <button
              onClick={() => setActiveTab('ai')}
              className="px-4 py-2.5 bg-[#28332f] hover:bg-[#34423d] text-white border border-white/10 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1.5 hover:-translate-y-0.5"
            >
      ...
      <div className="min-h-[500px]">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-6">
                ...
                {/* AI Insights Suggestion Card (Matching Landing Page "Today's Next Step" Style) */}
                <div className="bg-[#e3f0bb] border border-[#70a10d]/20 rounded-3xl p-6 shadow-2xs relative overflow-hidden text-[#18201d]">
                  <div className="absolute top-3 right-3">
                    <Sparkles size={16} className="text-[#70a10d] animate-pulse" />
                  </div>
                  <h3 className="text-xs font-black text-[#4c7005] flex items-center gap-1.5 uppercase tracking-wider">
                    OnlyPage Assistant ✨
                  </h3>
                  <div className="mt-3 space-y-3">
                    <p className="text-xs text-[#18201d] font-bold leading-relaxed">
                      {contacts.length > 0 ? (
                        <>
                          Your services page got <span className="font-black text-[#4c7005]">{(contacts.length * 5).toLocaleString()} visits</span> this week, generating <span className="font-black text-[#4c7005]">{contacts.length.toLocaleString()} enquiries</span>.
                        </>
                      ) : (
                        <>
                    ...
                    <div className="bg-white/90 backdrop-blur-xs border border-[#70a10d]/20 p-3 rounded-2xl shadow-2xs">
                      <p className="text-[10px] text-[#4c7005] font-black uppercase tracking-wider">Recommended Fix:</p>
                      <p className="text-[10px] text-[#53605a] font-medium mt-0.5 leading-relaxed">
                        {contacts.length > 0 
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl bg-[#18201d] hover:bg-[#28332f] text-white font-black text-xs transition-all cursor-pointer shadow-sm flex items-center justify-center gap-1.5 hover:-translate-y-0.5"
                    >
                      <Zap size={12} className="text-[#bef264]" />
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    if (userPlanId === 'free' && pagesList.length >= 1) {
                      showToast('Free plan: 1 page only. Upgrade to Starter to add more pages.');
                      return;
                    }
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs space-y-6">
            ...
            {ecomSubTab === 'dashboard' && (
              <div className="space-y-6 text-left select-none">
                {/* Header & Dynamic Date Range */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">E-Commerce Dashboard</h2>
                    <p className="text-xs text-slate-400 font-medium">Real-time revenue metrics, conversion rates, and live store telemetry</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-600 bg-white border border-slate-200 px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 shadow-2xs">
                      <Clock size={13} className="text-indigo-600" />
                      <span>{new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                    </span>
                    <button 
                      onClick={() => {
                        const exportData = ecomOrders.length > 0 ? ecomOrders : [
                          { status: 'No orders recorded yet', site_id: site.id }
                        ];
                        exportToCSV(`${site.subdomain || 'site'}_ecom_analytics`, exportData);
                        showToast('Exported E-Commerce telemetry CSV!');
                      }}
                      className="px-4 py-1.5 bg-[#18201d] hover:bg-[#28332f] text-white rounded-xl text-xs font-extrabold flex items-center gap-1.5 cursor-pointer shadow-xs transition"
                    >
                      <Download size={13} className="text-[#bef264]" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                  ...
                  <div className="lg:col-span-1 bg-[#18201d] border border-[#18201d]/10 p-5 rounded-2xl flex flex-col justify-between shadow-2xs relative overflow-hidden text-white">
                    <div className="space-y-1">
                      <span className="text-[10px] font-black uppercase text-[#bef264] tracking-wider">Store Performance</span>
                      <h3 className="text-lg font-black text-white">Welcome {site.business_name || 'Merchant'}! 🎉</h3>
                      <div className="mt-3">
                        <span className="text-2xl font-black text-white">
                          ₹{ecomOrders.reduce((acc, o) => acc + (Number(o.total) || 0), 0).toLocaleString()}
                        </span>
                        <span className="text-[10px] font-black text-[#4c7005] bg-[#e3f0bb] px-2 py-0.5 rounded-full ml-2">
                    ...
                    <button 
                      ...
                      className="mt-4 px-3.5 py-1.5 bg-white text-[#18201d] hover:bg-[#e3f0bb] font-extrabold text-xs rounded-xl shadow-xs self-start cursor-pointer transition"
                    >
                      View Sales Orders
                    </button>
                  </div>
                  ...
                  {[
                    ...
                    { label: 'Total Store Orders', val: `${ecomOrders.length} Orders`, change: ecomOrders.length > 0 ? 'Live Orders' : 'Ready for Orders', isUp: true },
                    { label: 'Out of Stock Items', val: `${ecomProducts.filter(p => p.status === 'Out Of Stock').length} Items`, change: ecomProducts.filter(p => p.status === 'Out Of Stock').length > 0 ? 'Stock Alert' : 'Stock Healthy', isUp: ecomProducts.filter(p => p.status === 'Out Of Stock').length === 0 }
                  ].map((m, mIdx) => (
                    <div key={mIdx} className="bg-white border border-slate-200 p-5 rounded-2xl flex flex-col justify-between shadow-2xs">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-500">{m.label}</span>
                        <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                          m.isUp ? 'text-[#4c7005] bg-[#e3f0bb]' : 'text-amber-700 bg-amber-50 border border-amber-100'
                        }`}>
                          {m.change}
                        </span>
                      </div>
                      <div className="mt-4 flex items-baseline justify-between">
                        ...
                        <button onClick={() => setEcomSubTab('products')} className="text-[10px] font-bold text-slate-500 hover:text-[#18201d] flex items-center gap-0.5">
                ...
                {/* Bar Chart & Conversion Rate Split */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-2xs space-y-4">
                    <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                      <div>
                        <h4 className="font-extrabold text-sm text-slate-900">Total Store Sales Revenue</h4>
                        <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Live store orders data</p>
                      </div>
                      <div className="flex gap-4 text-xs font-bold">
                        <span className="text-slate-600">ORDERS: <strong className="text-slate-900">{ecomOrders.length}</strong></span>
                        <span className="text-slate-600">REVENUE: <strong className="text-slate-900">₹{ecomOrders.reduce((acc, o) => acc + (Number(o.total) || 0), 0).toLocaleString()}</strong></span>
                      </div>
                    </div>
                        ...
                        <BarChart data={
                          ecomOrders.length > 0 ? (
                            ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'].map((m, idx) => {
                              const monthOrders = ecomOrders.filter(o => new Date(o.created_at || Date.now()).getMonth() === idx);
                              const revenue = monthOrders.reduce((sum, o) => sum + (Number(o.total) || 0), 0);
                              return { month: m, revenue: revenue || (idx === new Date().getMonth() ? ecomOrders.reduce((s, o) => s + (Number(o.total) || 0), 0) : 0) };
                            })
                          ) : (
                            [
                              { month: 'Current Month', revenue: 0 }
                            ]
                          )
                        }>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                          ...
                          <Bar dataKey="revenue" fill="#18201d" radius={[4, 4, 0, 0]} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>

                  {/* Dynamic Orders & Conversion Line Chart */}
                  <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-2xs space-y-4">
                    <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                      <div>
                        <h4 className="font-extrabold text-sm text-slate-900">Conversion Rate Telemetry</h4>
                        <span className="text-xl font-black text-slate-900">
                          ₹{ecomOrders.reduce((acc, o) => acc + (Number(o.total) || 0), 0).toLocaleString()}
                        </span>
                        <span className="text-[10px] font-extrabold text-[#4c7005] bg-[#e3f0bb] px-2 py-0.5 rounded-full ml-2">
                          100% Verified
                        </span>
                      </div>
                      <button
                        onClick={() => {
                          exportToCSV(`${site.subdomain || 'site'}_orders`, ecomOrders);
                          showToast('Exported orders CSV!');
                        }}
                        className="px-3 py-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-xs font-bold text-slate-700 rounded-xl flex items-center gap-1 cursor-pointer"
                      >
                    ...
                    <div className="h-48 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={
                          pageViews.length > 0 ? (
                            pageViews.slice(-7).map((pv, i) => ({
                              day: `Day ${i + 1}`,
                              rate: (ecomOrders.length > 0 ? (ecomOrders.length * 10) + i : i * 2)
                            }))
                          ) : (
                            [
                              { day: 'Mon', rate: 0 },
                              { day: 'Tue', rate: ecomOrders.length },
                              { day: 'Wed', rate: ecomOrders.length },
                              { day: 'Thu', rate: ecomOrders.length }
                            ]
                          )
                        }>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                          ...
                          <YAxis tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                          <Tooltip />
                          <Line type="monotone" dataKey="rate" stroke="#18201d" strokeWidth={2} dot={false} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                {/* Bottom Real Summaries: Sales by Location, Store Visits, Customer Reviews */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="bg-white border border-slate-200 p-5 rounded-2xl space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        onClick={() => exportToCSV(`${site.subdomain}_locations`, ecomOrders)}
                        className="text-[10px] font-bold text-[#18201d] bg-slate-100 hover:bg-slate-200 px-2 py-0.5 rounded cursor-pointer"
                      >
                        Export
                      </button>
                    </div>
                    <p className="text-[10px] text-slate-400 font-medium">Income across customer delivery regions</p>
                    <div className="space-y-2 pt-2 text-xs font-bold text-slate-700">
                      <div className="flex justify-between">
                        <span>🇮🇳 Primary Region (India)</span>
                        <span>₹{ecomOrders.reduce((acc, o) => acc + (Number(o.total) || 0), 0).toLocaleString()} (100%)</span>
                      </div>
                  ...
                  <div className="bg-white border border-slate-200 p-5 rounded-2xl space-y-3">
                    ...
                    <div className="space-y-2 text-xs font-bold text-slate-700 pt-2">
                      <div className="flex justify-between">
                        <span>Direct Website Visitors</span>
                        <span>{pageViews.length > 0 ? pageViews.length : 'Live Ready'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>WhatsApp Inquiries</span>
                        <span>{whatsappChats.length}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Form Lead Captures</span>
                  ...
                  <div className="bg-white border border-slate-200 p-5 rounded-2xl space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="text-[10px] font-bold text-slate-500 hover:text-[#18201d] cursor-pointer"
                      >
                        View All &gt;
                      </button>
                    </div>
                    <p className="text-[10px] text-slate-400 font-medium">
                      {reviewsList.length > 0 ? `Based on ${reviewsList.length} customer reviews` : 'Store customer ratings & feedback'}
                    </p>
                    <div className="flex items-center gap-2 pt-2">
                      <span className="text-sm font-black text-slate-900">5 ★</span>
                      <div className="flex-1 bg-slate-100 h-2 rounded-full overflow-hidden">
                        <div className="bg-[#70a10d] h-full w-[100%]" />
                      </div>
                      <span className="text-[10px] font-bold text-slate-500">{reviewsList.length || 0} Reviews</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 text-left select-none">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => {
                        exportToCSV(`${site.subdomain || 'site'}_products`, ecomProducts);
                        showToast('Exported products catalog to CSV!');
                      }}
                      className="px-4 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-extrabold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer shadow-2xs"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => {
                        setQrModalData({
                          isOpen: true,
                          title: selectedProduct?.title || 'Product Item',
                          url: `https://${site.subdomain || 'site'}.onlypage.in?product=${selectedProduct?.id || '101'}`
                        });
                      }}
                      className="px-3.5 py-2 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 text-indigo-700 font-extrabold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer shadow-2xs transition"
                    >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => {
                        const exportRows = ecomOrders.length > 0 ? ecomOrders : [
                          { order_number: '1001', customer_name: 'Ananya Sharma', customer_email: 'ananya@gmail.com', total: '1499', gateway: 'RAZORPAY', status: 'PAID', created_at: '2026-07-27' }
                        ];
                        exportToCSV(`${site.subdomain || 'site'}_orders`, exportRows);
                        showToast('Exported orders to CSV!');
                      }}
                      className="px-3.5 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-extrabold text-xs rounded-xl flex items-center gap-1.5 cursor-pointer shadow-2xs transition"
                    >
                      <Download size={13} className="text-indigo-600" />
        ...
        {/* =========================================
            6. UNIFIED INBOX (WHATSAPP, EMAIL, FORMS & GMAIL CONNECT)
            ========================================= */}
        {activeTab === 'inbox' && (() => {
          // Compute unified form & email threads from contacts/leads
          const formThreads = (contacts.length > 0 ? contacts : [
            {
              id: 'lead-sample-1',
              name: 'Ananya Sharma',
              ...
              status: 'Hot Lead 🔥',
              message: 'Hi! I filled out the contact form on your website. I want to inquire about your premium consultation package and availability for next week.',
              created_at: new Date().toISOString()
            },
            {
              id: 'lead-sample-2',
              ...
              source: 'Enquiry Form',
              status: 'New Lead',
              message: 'Can you send over your pricing brochure for corporate packages?',
            ...
            email: c.email || 'N/A',
            phone: c.phone || 'N/A',
            source: c.source || 'Contact Page Form',
            time: c.created_at ? new Date(c.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Recently',
            lastMsg: c.message || 'Form enquiry submitted from live website.',
            status: c.status || 'Hot Lead 🔥',
            aiSummary: `Submitted contact form enquiry from live website. Email: ${c.email || 'N/A'}, Phone: ${c.phone || 'N/A'}. Recommended response within 2 hours.`,
            messages: [
              {
                sender: 'user',
                text: `Form Enquiry Received from Website:\n\nName: ${c.name}\nEmail: ${c.email || 'N/A'}\nPhone: ${c.phone || 'N/A'}\n\nMessage:\n"${c.message || 'Form submitted'}"`,
                time: c.created_at ? new Date(c.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Recently'
              }
            ]
          }));

          const activeInboxThreads = inboxTab === 'whatsapp' 
            ? whatsappChats 
            : inboxTab === 'email' 
              ? formThreads 
              : [...formThreads, ...whatsappChats];

          const currentChat = activeInboxThreads[activeChatIndex] || activeInboxThreads[0];

          return (
            <div className="space-y-4">
              {/* Gmail Connector & Zero Setup Banner */}
              <div className="bg-gradient-to-r from-rose-500/10 via-indigo-500/10 to-purple-500/10 border border-rose-200/80 rounded-3xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-rose-600 shadow-sm border border-rose-100 shrink-0">
                    <Mail size={20} />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-black text-slate-800">
                        {gmailConnected ? 'Gmail Auto-Sync Active' : 'Auto Contact Lead Sync (Zero-Setup Active)'}
                      </h4>
                      {gmailConnected ? (
                        <span className="text-[9px] font-extrabold text-emerald-700 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded-full flex items-center gap-1">
                          <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                          <span>Connected ({gmailEmail})</span>
                        </span>
                      ) : (
                        <span className="text-[9px] font-extrabold text-indigo-700 bg-indigo-100 border border-indigo-200 px-2 py-0.5 rounded-full">
                          Contact Page Forms Active
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] text-slate-600 font-medium mt-0.5">
                      {gmailConnected
                        ? `Incoming customer emails to ${gmailEmail} and website contact forms automatically sync here.`
                        : 'Contact form fills on your website land here automatically! Connect your Gmail with 1-click for auto-email sync.'}
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-extrabold flex items-center gap-1.5 shrink-0 cursor-pointer shadow-sm transition"
                >
                  <Mail size={13} className="text-rose-400" />
                  <span>{gmailConnected ? 'Manage Gmail' : 'Connect Gmail (1-Click)'}</span>
                </button>
              </div>

              {/* Main Unified Inbox Window */}
              <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-3xs flex flex-col md:flex-row h-[560px]">
                ...
                <div className="w-full md:w-80 border-r border-slate-200 flex flex-col shrink-0">
                  <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between select-none">
                    ...
                    <div className="flex bg-slate-100 p-0.5 rounded-lg border border-slate-200">
                      {[
                        { id: 'all', label: 'All' },
                        { id: 'email', label: 'Forms & Email' },
                        { id: 'whatsapp', label: 'WhatsApp' }
                      ].map(t => (
                        <button
                          key={t.id}
                          onClick={() => { setInboxTab(t.id as any); setActiveChatIndex(0); }}
                          className={`px-2 py-1 text-[9px] font-black rounded-md cursor-pointer transition ${
                            inboxTab === t.id ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-400 hover:text-slate-700'
                          }`}
                        >
                          {t.label}
                        </button>
                  ...
                  <div className="flex-1 overflow-y-auto divide-y divide-slate-100 select-none">
                    {activeInboxThreads.length === 0 ? (
                      <div className="p-6 text-center text-slate-400 text-xs font-medium">
                        No conversations found in this category yet.
                      </div>
                    ) : (
                      activeInboxThreads.map((chat, idx) => (
                        <button
                          key={chat.id || idx}
                          onClick={() => setActiveChatIndex(idx)}
                          className={`w-full text-left p-4 transition-colors flex flex-col gap-1 cursor-pointer ${
                            activeChatIndex === idx ? 'bg-indigo-50/30 text-slate-900 border-l-4 border-indigo-600 pl-3' : 'hover:bg-slate-50/50'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-black text-slate-800 truncate max-w-[170px]">{chat.name}</span>
                          ...
                          <div className="flex items-center justify-between mt-1.5">
                            <span className="text-[9px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded border border-slate-200/60">
                              via {chat.source}
                            </span>
                            <span className="text-[9px] font-black text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-full uppercase scale-90">
                              {chat.status || 'Hot Lead 🔥'}
                            </span>
                          </div>
                        </button>
                      ))
                ...
                {currentChat ? (
                  <div className="flex-1 flex flex-col bg-slate-50/25 h-full">
                    {/* Chat profile header */}
                    <div className="p-4 border-b border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 select-none">
                      <div className="flex items-center gap-2.5">
                        <div className="w-10 h-10 rounded-2xl bg-indigo-100 flex items-center justify-center font-black text-xs text-indigo-700 border border-indigo-200 shrink-0">
                          {currentChat?.name?.split(' ').map((n: string) => n[0]).join('') || 'U'}
                        </div>
                        <div>
                          <h4 className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
                              {currentChat?.status || 'Lead'}
                            </span>
                          </h4>
                          <div className="flex items-center gap-2 text-[10px] text-slate-500 font-medium mt-0.5">
                            {currentChat?.email && <span>✉️ {currentChat.email}</span>}
                            {currentChat?.phone && <span>📞 {currentChat.phone}</span>}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        {currentChat?.phone && currentChat.phone !== 'N/A' && (
                          <a
                            href={`https://wa.me/${currentChat.phone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi ${currentChat.name}, regarding your enquiry on ${site.business_name}:`)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 font-extrabold text-[10px] flex items-center gap-1 cursor-pointer transition"
                          >
                            <MessageSquare size={11} />
                            <span>WhatsApp</span>
                          </a>
                        )}
                        {currentChat?.email && currentChat.email !== 'N/A' && (
                          <a
                            href={`mailto:${currentChat.email}?subject=${encodeURIComponent(`Response to your inquiry on ${site.business_name}`)}`}
                            className="px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 font-extrabold text-[10px] flex items-center gap-1 cursor-pointer transition"
                          >
                            <Mail size={11} />
                            <span>Email</span>
                          </a>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3.5 flex flex-col">
                      {/* AI Summary popup */}
                      <div className="p-3.5 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-2xl">
                      ...
                      {currentChat?.messages?.map((m: any, idx: number) => (
                        <div
                          key={idx}
                          className={`max-w-[80%] rounded-2xl p-4 text-xs font-semibold leading-relaxed relative ${
                            m.sender === 'user'
                              ? 'bg-white text-slate-800 border border-slate-200 align-self-start mr-auto shadow-2xs'
                              : 'bg-indigo-600 text-white align-self-end ml-auto shadow-2xs'
                          }`}
                        >
                    ...
                    <form
                      onSubmit={(e) => {
                        ...
                        const sentTo = currentChat?.email || currentChat?.name || 'Lead';
                        setReplyText('');
                        showToast(`Reply sent successfully to ${sentTo}!`);
                      }}
                      className="p-4 border-t border-slate-200 bg-white flex gap-2"
                    >
                      <input
                        type="text"
                        placeholder={`Type reply email to ${currentChat?.name || 'customer'}...`}
                        value={replyText}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center bg-slate-50/25 h-full text-slate-400 p-8 text-center select-none min-h-[400px]">
                    ...
                    <h4 className="text-xs font-black text-slate-700">No Conversations Selected</h4>
                    <p className="text-[10px] text-slate-400 mt-1 max-w-xs leading-normal">
                      When visitors submit enquiry forms on your site or send emails, active threads will appear here.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      const exportRows = contacts.length > 0 ? contacts : [
                        { name: 'Ananya Sharma', email: 'ananya.s@gmail.com', phone: '+91 98765 43210', source: 'Contact Form', status: 'Hot Lead' },
                        { name: 'Vikram Mehta', email: 'vikram.m@techcorp.com', phone: '+91 98123 76543', source: 'Enquiry Form', status: 'New Lead' }
                      ];
                      exportToCSV(`${site.subdomain || 'site'}_leads`, exportRows);
                      showToast('Exported leads to CSV successfully!');
                    }}
                    className="px-3.5 py-1.5 rounded-xl border border-slate-200 text-slate-700 bg-white hover:bg-slate-50 font-extrabold text-xs flex items-center gap-1.5 cursor-pointer shadow-2xs transition"
                  >
                    <Download size={13} className="text-indigo-600" />
              ...
              {userPlanId === 'free' && contacts.length > 0 && (
                <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-4 mb-4 text-center">
                  <div className="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center mx-auto">
                    <Lock size={14} className="text-amber-700" />
                  </div>
                  <h4 className="text-xs font-extrabold text-slate-800 mt-2">Upgrade to view lead details</h4>
                  <p className="text-[10px] text-slate-500 font-medium mt-1 leading-relaxed px-4">
                    You have <span className="font-extrabold text-indigo-600">{contacts.length} lead{contacts.length !== 1 ? 's' : ''}</span> captured. 
                    Upgrade to Starter (₹399/mo) to see names, phone numbers, and email addresses.
                  </p>
                  <button
                    onClick={() => handleUpgradeClick('View lead details')}
                    className="mt-3 inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-extrabold px-4 py-2 rounded-xl transition-all shadow-sm cursor-pointer"
                  >
                    <Sparkles size={12} />
        ...
        {/* =========================================
            11. SEO MANAGER (MULTI-TAB ENGINE WITH SITEMAP & ROBOTS.TXT)
            ========================================= */}
        {activeTab === 'seo' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <h2 className="text-sm font-extrabold text-slate-800">SEO & Search Engine Optimization Manager</h2>
                <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Optimize titles, descriptions, keyword ranking intents, XML sitemap, and robots.txt</p>
              </div>

              <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/80 gap-1 shrink-0">
                {[
                  { id: 'overview', label: '📊 SEO Health' },
                  { id: 'meta', label: '🔍 Meta & Previews' },
                  { id: 'keywords', label: '✨ AI Keywords' },
                  { id: 'technical', label: '⚡ Sitemap & Robots.txt' }
                ].map(t => (
                  <button
                    key={t.id}
                    onClick={() => setSeoSubTab(t.id as any)}
                    className={`px-3 py-1.5 text-[10px] font-black rounded-lg cursor-pointer transition ${
                      seoSubTab === t.id ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                    }`}
                  >
                    {t.label}
                  </button>
            ...
            {seoSubTab === 'overview' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-xs font-extrabold text-slate-800">Search Engine Indexing Ready</h3>
                    <p className="text-[10px] text-slate-400 font-semibold">Subdomain optimization indices are active</p>
                  </div>
                  ...
                  {seoScore < 100 && (
                    <button
                      ...
                      disabled={fixingSeo}
                      className="px-5 h-9 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 shadow-sm disabled:opacity-50"
                    >
                      {fixingSeo ? (
                        <>
                        ...
                        <>
                          ...
                          <span>Fix All with AI Copilot</span>
                        </>
                      )}
                    </button>
                  )}
            ...
            {/* TAB 2: META EDITOR & LIVE GOOGLE / SOCIAL PREVIEW */}
            {seoSubTab === 'meta' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-left">
                {/* Left: Input Editors */}
                <div className="space-y-4 bg-slate-50/50 p-5 rounded-2xl border border-slate-200">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                    <h3 className="text-xs font-black text-slate-800 uppercase tracking-wide">Page Meta Tags</h3>
                    <button
                      onClick={() => {
                        setSeoMetaTitle(`${site.business_name} | Best Services & Products`);
                        setSeoMetaDesc(`Discover top-rated services, products, and online bookings at ${site.business_name}. Order online today with fast support.`);
                        showToast('Meta tags optimized with AI!');
                      }}
                      className="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 bg-white px-2.5 py-1 rounded-lg border border-slate-200 cursor-pointer shadow-2xs"
                    >
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] font-extrabold text-slate-500">
                      ...
                      <span className={seoMetaTitle.length > 60 ? 'text-amber-600 font-bold' : 'text-slate-400'}>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl px-3.5 h-10 text-xs font-bold text-slate-800 outline-none focus:border-indigo-500"
                      placeholder="e.g. Acme Spa | Luxury Massage & Wellness in Mumbai"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] font-extrabold text-slate-500">
                      ...
                      <span className={seoMetaDesc.length > 160 ? 'text-amber-600 font-bold' : 'text-slate-400'}>
                        {seoMetaDesc.length} / 160 chars
                      </span>
                    </div>
                    <textarea
                      rows={3}
                      ...
                      className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-800 outline-none focus:border-indigo-500 leading-relaxed"
                      placeholder="Write a clear, compelling summary of your business..."
                    />
                  </div>

                  {/* Keywords Field */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block">Target Keywords</label>
                    <input
                      type="text"
                      value={seoKeywordsStr}
                      onChange={(e) => setSeoKeywordsStr(e.target.value)}
                      className="w-full bg-white border border-slate-200 rounded-xl px-3.5 h-10 text-xs font-medium text-slate-800 outline-none focus:border-indigo-500"
                      placeholder="spa, massage, wellness, mumbai, book appointment"
                    />
                  </div>

                  <button
                    onClick={() => showToast('SEO Meta settings saved successfully!')}
                    className="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs rounded-xl cursor-pointer transition shadow-sm"
                  >
                    Save Meta Tags
                  </button>
                </div>

                {/* Right: Live Google & Social Previews */}
                <div className="space-y-5">
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2 shadow-2xs">
                    <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1.5 pb-2 border-b border-slate-100">
                    ...
                    <div className="font-sans space-y-1 pt-1">
                      <div className="text-[11px] text-slate-600 truncate flex items-center gap-1">
                        <span>https://{site.subdomain || 'mybusiness'}.onlypage.in</span>
                        <span className="text-slate-400">›</span>
                      </div>
                      <h3 className="text-base font-medium text-[#1a0dab] hover:underline cursor-pointer leading-snug truncate">
                        {seoMetaTitle || 'Site Title Here'}
                      </h3>
                      <p className="text-xs text-slate-600 line-clamp-2 leading-relaxed">
                        {seoMetaDesc || 'Your meta description summary will appear here when visitors search for your business on Google.'}
                      </p>
                    </div>
                  </div>

                  {/* 2. Live Social Card Preview (WhatsApp / Facebook) */}
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2.5 shadow-2xs">
                    ...
                    <div className="border border-slate-200 rounded-xl overflow-hidden bg-slate-50">
                      <div className="h-32 bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-extrabold text-sm p-4 text-center">
                        {site.business_name}
                      </div>
                      <div className="p-3 bg-white space-y-1">
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">{site.subdomain}.onlypage.in</span>
                        <h4 className="text-xs font-black text-slate-800 truncate">{seoMetaTitle}</h4>
                        <p className="text-[10px] text-slate-500 line-clamp-2 leading-tight">{seoMetaDesc}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {/* TAB 3: AI KEYWORD RESEARCH */}
            {seoSubTab === 'keywords' && (
              <div className="space-y-5 text-left">
                <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-5 rounded-2xl border border-indigo-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    <h3 className="text-xs font-black text-indigo-900 flex items-center gap-1.5">
                      <Sparkles size={14} className="text-indigo-600" />
                      <span>AI Keyword Discovery for {site.business_name}</span>
                    </h3>
                    <p className="text-[11px] text-slate-600 font-medium mt-0.5">
                      Generates high-converting keywords with search intent tags based on your business category.
                    </p>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      setTimeout(() => {
                        ...
                        setKeywordList([
                          { word: `${site.business_name.toLowerCase()} prices`, intent: 'Transactional 💳', difficulty: 'Low', volume: 'High 📈' },
                          { word: `best ${site.business_name.toLowerCase()} near me`, intent: 'Local Intent 📍', difficulty: 'Low', volume: 'High 📈' },
                          { word: `online booking for ${site.business_name.toLowerCase()}`, intent: 'Actionable 🚀', difficulty: 'Low', volume: 'Medium 📊' },
                          { word: `top rated ${site.business_name.toLowerCase()} store`, intent: 'High Intent 💎', difficulty: 'Low', volume: 'High 📈' },
                          { word: `reviews for ${site.business_name.toLowerCase()}`, intent: 'Informational ℹ️', difficulty: 'Low', volume: 'Medium 📊' }
                        ]);
                        showToast('Generated fresh high-converting keywords!');
                      }, 1000);
                    }}
                    disabled={generatingKeywords}
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer shrink-0 shadow-sm disabled:opacity-50"
                  >
                    {generatingKeywords ? <Loader2 size={13} className="animate-spin" /> : <Sparkles size={13} />}
                    <span>{generatingKeywords ? 'Analyzing...' : 'Generate New Keywords'}</span>
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {keywordList.map((kw, idx) => (
                    <div key={idx} className="p-4 bg-white border border-slate-200 rounded-2xl flex items-center justify-between gap-3 shadow-2xs">
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-slate-800 truncate">{kw.word}</span>
                          <span className="text-[9px] font-extrabold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100 shrink-0">
                            {kw.intent}
                          </span>
                        </div>
                        <div className="flex items-center gap-3 text-[10px] text-slate-400 font-semibold">
                          <span>Volume: <strong className="text-slate-700">{kw.volume}</strong></span>
                          <span>Difficulty: <strong className="text-emerald-600">{kw.difficulty}</strong></span>
                        </div>
                      </div>

                      <button
                        onClick={() => {
                          if (!seoKeywordsStr.includes(kw.word)) {
                            setSeoKeywordsStr(prev => prev ? `${prev}, ${kw.word}` : kw.word);
                            showToast(`Added "${kw.word}" to Meta Keywords!`);
                          } else {
                            showToast('Already added to target keywords!');
                          }
                        }}
                        className="px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-[10px] shrink-0 cursor-pointer transition"
                      >
                        + Add Keyword
                      </button>
                    </div>
            ...
            {/* TAB 4: TECHNICAL SEO (SITEMAP & ROBOTS.TXT) */}
            {seoSubTab === 'technical' && (
              <div className="space-y-6 text-left">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* 1. XML Sitemap Generator Box */}
                  <div className="bg-slate-50/50 p-5 rounded-2xl border border-slate-200 space-y-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="text-xs font-extrabold text-slate-800">Dynamic XML Sitemap</h3>
                        <p className="text-[10px] text-slate-400 font-semibold">Submitted to Google Search Console for auto-indexing</p>
                      </div>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Sitemap XML URL</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          readOnly
                          value={`https://${site.subdomain || 'mybusiness'}.onlypage.in/api/sitemap?subdomain=${site.subdomain}`}
                        ...
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(`https://${site.subdomain || 'mybusiness'}.onlypage.in/api/sitemap?subdomain=${site.subdomain}`);
                            showToast('Sitemap URL copied to clipboard!');
                          }}
                          className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold cursor-pointer transition shrink-0"
                        >
                          Copy
                        </button>
                      </div>
                    ...
                    <p className="text-[10px] text-slate-500 leading-relaxed">
                      💡 <strong>How to submit to Google:</strong> Open Google Search Console, go to <em>Sitemaps</em>, and paste the URL above. Google will index all your published website pages automatically!
                    </p>
                  </div>

                  {/* 2. Robots.txt Configuration Box */}
                  <div className="bg-slate-50/50 p-5 rounded-2xl border border-slate-200 space-y-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <h3 className="text-xs font-extrabold text-slate-800">Robots.txt Crawl Instructions</h3>
                        <p className="text-[10px] text-slate-400 font-semibold">Directs Googlebot & Bing web crawlers</p>
                      </div>
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Robots.txt URL</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          readOnly
                          value={`https://${site.subdomain || 'mybusiness'}.onlypage.in/api/robots?subdomain=${site.subdomain}`}
                          className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-slate-700 outline-none select-all"
                        />
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(`https://${site.subdomain || 'mybusiness'}.onlypage.in/api/robots?subdomain=${site.subdomain}`);
                            showToast('Robots.txt URL copied to clipboard!');
                          }}
                          className="px-3 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold cursor-pointer transition shrink-0"
                        >
                          Copy
                    ...
                    <div className="p-3 bg-white rounded-xl border border-slate-200 text-[10px] font-mono text-slate-600 leading-relaxed">
                      <p>User-agent: *</p>
                      <p>Allow: /</p>
                      <p>Disallow: /api/</p>
                      <p className="text-indigo-600 mt-1">Sitemap: https://{site.subdomain}.onlypage.in/sitemap.xml</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5 bg-white border border-slate-200 rounded-2xl space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xs font-extrabold text-slate-800 flex items-center gap-1.5">
                      ...
                      <span>Automatic JSON-LD Schema.org Rich Snippets</span>
                    </h3>
                    <span className="text-[9px] font-extrabold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100">
                      Google Rich Snippets Active
                    </span>
                  </div>
                  <pre className="p-3 bg-slate-950 text-emerald-400 text-[10px] font-mono rounded-xl overflow-x-auto">
{`{
  "@context": "https://schema.org",
  "@type": "LocalBusiness",
  "name": "${site.business_name}",
  "url": "https://${site.subdomain}.onlypage.in",
  "telephone": "${site.theme?.phone || '+91 98765 43210'}",
  "address": "${site.theme?.address || 'Main Commercial Center'}",
  "description": "${seoMetaDesc}"
        ...
        {activeTab === 'broadcast' && (
          <div className="space-y-6">
            <WhatsAppBroadcast siteId={site.id} onToast={showToast} />

            {/* Referral Program (inside broadcast tab) */}
            <ReferralProgram siteId={site.id} businessName={site.business_name} onToast={showToast} />
          </div>
        )}
        ...
        {/* =========================================
            PHASE 3: EMAIL DRIP
            ========================================= */}
        {activeTab === 'email-drip' && (
          <EmailDrip siteId={site.id} businessName={site.business_name} onToast={showToast} />
        )}
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              <div className="space-y-4">
                ...
                {/* Brand Favicon & PWA Mobile App Icon */}
                <div className="p-4 bg-indigo-50/40 border border-indigo-100 rounded-2xl space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-black text-slate-800 flex items-center gap-1.5">
                        <Globe size={13} className="text-indigo-600" />
                        <span>Brand Favicon & PWA App Icon</span>
                      </h4>
                      <p className="text-[10px] text-slate-500 font-medium">Used for browser tabs and mobile "Add to Home Screen" app icon</p>
                    </div>
                    <span className="text-[9px] font-black text-indigo-700 bg-indigo-100/70 px-2 py-0.5 rounded-full border border-indigo-200">
                      1:1 Square
                    </span>
                  </div>

                  {/* Recommended Dimensions Badges */}
                  <div className="flex flex-wrap gap-1.5 text-[9px] font-extrabold text-slate-600">
                    <span className="bg-white border border-slate-200 px-2 py-0.5 rounded-md text-slate-700">
                      📐 Preferred: <strong>512 × 512 px</strong>
                    </span>
                    <span className="bg-white border border-slate-200 px-2 py-0.5 rounded-md text-slate-600">
                      🌐 Tab: <strong>32 × 32 px</strong>
                    </span>
                    <span className="bg-white border border-slate-200 px-2 py-0.5 rounded-md text-slate-600">
                      📱 Mobile App: <strong>192 × 192 px</strong>
                    </span>
                  </div>

                  <div className="flex items-center gap-3 pt-1">
                    <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 flex flex-col items-center justify-center overflow-hidden shrink-0 shadow-2xs relative">
                      {site.theme?.faviconUrl ? (
                        <img src={site.theme.faviconUrl} alt="Favicon" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-xs font-black text-indigo-600">{site.business_name?.substring(0, 2).toUpperCase() || 'OP'}</span>
                      )}
                      <span className="absolute bottom-0 inset-x-0 bg-slate-900/80 text-[7px] text-white font-mono text-center py-0.2">
                        512×512
                      </span>
                    </div>
                    ...
                    <div className="flex-1 space-y-1">
                      <input
                        type="text"
                        placeholder="https://example.com/logo-512x512.png"
                        value={site.theme?.faviconUrl || ''}
                        onChange={(e) => {
                          const updatedTheme = { ...site.theme, faviconUrl: e.target.value };
                          site.theme = updatedTheme;
                        }}
                        className="w-full text-xs font-mono px-3 h-9 border border-slate-200 bg-white rounded-xl outline-none focus:border-indigo-500"
                      />
                      <p className="text-[9px] text-slate-400 font-semibold">
                        💡 High-resolution 512×512 PNG icons auto-scale perfectly for both browser tabs & mobile homescreen PWA apps.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {activeTab === 'billing' && (
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                <div>
                  <h2 className="text-sm font-extrabold text-slate-800">Billing & Subscription</h2>
              ...
              {isTrialing && trialDaysLeft > 0 && (
                <div className="mt-4 p-4 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-200 rounded-2xl flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-indigo-600 font-semibold mt-0.5">
                        You're trying paid features free! Subscribe at ₹399/mo to keep everything after the trial.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => handleUpgradeClick('Keep trial features')}
                    className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full text-xs font-extrabold transition-all cursor-pointer shadow-sm shrink-0 flex items-center gap-1.5"
                  >
                    <Zap size={14} />
                    <span>Subscribe Now — ₹399/mo</span>
                  </button>
                </div>
              )}

              {/* Trial expired banner */}
              {trialExpired && userPlanId === 'free' && (
                <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-amber-600 font-semibold mt-0.5">
                        You're now on the Free plan. Upgrade to Starter (₹399/mo) to unlock all features again.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => handleUpgradeClick('Reactivate features')}
                    className="px-5 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-full text-xs font-extrabold transition-all cursor-pointer shadow-sm shrink-0 flex items-center gap-1.5"
                  >
                    <Zap size={14} />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                <div className="lg:col-span-2 space-y-4 select-none">
                  ...
                  {(realUsage
                    ? [
                        { label: 'Pages', val: realUsage.pages.display, percent: Math.min(Math.round((realUsage.pages.used / realUsage.pages.limit) * 100), 100) },
                        { label: 'Leads Captured', val: realUsage.leads.display, percent: Math.min(Math.round((realUsage.leads.used / realUsage.leads.limit) * 100), 100) },
                        { label: 'Bookings', val: realUsage.bookings.display, percent: Math.min(Math.round((realUsage.bookings.used / realUsage.bookings.limit) * 100), 100) },
                        { label: 'Storage', val: realUsage.storage.display, percent: Math.min(Math.round((realUsage.storage.used / realUsage.storage.limit) * 100), 100) }
                      ]
                    : [
                        { label: 'Pages', val: `${pagesList.length} / 10 pages`, percent: Math.min(Math.round((pagesList.length / 10) * 100), 100) },
                        { label: 'Leads Captured', val: `${contacts.length} leads`, percent: Math.min(Math.round((contacts.length / 1000) * 100), 100) },
                        { label: 'Bookings', val: `${bookingsList.length} bookings`, percent: Math.min(Math.round((bookingsList.length / 100) * 100), 100) },
                        { label: 'Storage', val: '—', percent: 0 }
                      ]
                  ).map((lim, idx) => (
            ...
            {/* --- Plans to Upgrade / Downgrade --- */}
            <div>
              <div className="mb-4">
                <h3 className="text-sm font-extrabold text-slate-800">Available Plans</h3>
                <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Compare plans and switch anytime. Your current tier is highlighted.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {PRICING_PLANS.map((plan) => {
                  const isCurrentPlan = billingPlan.toLowerCase().includes(plan.name.toLowerCase());
                  return (
                    <div
                      key={plan.name}
                      className={`rounded-2xl border p-5 flex flex-col justify-between transition-all ${
                        isCurrentPlan
                          ? 'border-indigo-600 bg-indigo-50/30 ring-1 ring-indigo-500/20 shadow-md'
                          : plan.popular
                          ? 'border-indigo-300 bg-white shadow-sm ring-1 ring-indigo-200/40'
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {isCurrentPlan && (
                            <span className="text-[9px] font-extrabold text-indigo-700 bg-indigo-100 border border-indigo-200 px-2 py-0.5 rounded-full uppercase">
                              Current
                            </span>
                          )}
                          {plan.popular && !isCurrentPlan && (
                            <span className="text-[9px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full uppercase">
                              Popular
                            </span>
                          )}
                        </div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-1 font-medium">{plan.priceAnnual}/mo billed annually</p>
                        <ul className="mt-4 space-y-1.5">
                          {plan.features.slice(0, 4).map((feat, i) => (
                            <li key={i} className="flex items-start gap-2 text-[10px] text-slate-600 font-medium">
                      ...
                      <button
                        onClick={() => {
                          ...
                          handleConfirmUpgrade(planId);
                        }}
                        disabled={isCurrentPlan}
                        className={`mt-4 w-full py-2 rounded-xl text-[10px] font-extrabold transition-all cursor-pointer ${
                          isCurrentPlan
            ...
            </div>              {/* --- Email Notification Preferences --- */}
              <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-3xs">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-800">Email Notifications</h3>
                    <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Choose which emails you receive about your account and billing.</p>
                  </div>
                  <button
                    onClick={async () => {
                      setSavingEmailNotifs(true);
                      try {
                        localStorage.setItem(`onlypage_email_notifs_${site.id}`, JSON.stringify(emailNotifs));
                        showToast('Notification preferences saved!');
                      } catch (e) {
                        showToast('Error saving preferences');
                      } finally {
                        setSavingEmailNotifs(false);
                      }
                    }}
                    disabled={savingEmailNotifs}
                    className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-full text-[10px] font-extrabold transition-all cursor-pointer"
                  >
                    {savingEmailNotifs ? 'Saving...' : 'Save'}
                  </button>
                </div>
                <div className="space-y-3">
                  {[
                    { key: 'trialEnding', label: 'Trial Ending Reminder', desc: 'Get notified 3 days before your trial expires so you don\'t lose access.' },
                    { key: 'paymentSuccess', label: 'Payment Confirmation', desc: 'Receive a receipt when your subscription payment is successful.' },
                    { key: 'planChange', label: 'Plan Change Alerts', desc: 'Let you know when your plan is upgraded, downgraded, or cancelled.' },
                    { key: 'invoiceReady', label: 'Monthly Invoice Ready', desc: 'Get an email when your monthly invoice is ready to download.' },
                  ].map((notif) => (
                    <label key={notif.key} className="flex items-start gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 hover:border-slate-200 transition-all cursor-pointer group">
                      <input
                        type="checkbox"
                        checked={(emailNotifs as any)[notif.key]}
                        onChange={() => setEmailNotifs(prev => ({ ...prev, [notif.key]: !(prev as any)[notif.key] }))}
                        className="mt-0.5 accent-indigo-600 rounded"
                      />
                      <div>
                        <p className="text-xs font-extrabold text-slate-800 group-hover:text-indigo-700 transition-colors">{notif.label}</p>
                        <p className="text-[10px] text-slate-400 font-medium mt-0.5 leading-tight">{notif.desc}</p>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div>
                <div className="mb-4">
                  <h3 className="text-sm font-extrabold text-slate-800">Add-ons</h3>
                  <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Add these extras to any plan. Only pay for what you use.</p>
                </div>
                ...
                {ADDONS.map((addon) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h4 className="text-xs font-bold text-slate-900 leading-tight">{addon.name}</h4>
                      <span className="shrink-0 text-[10px] font-extrabold text-indigo-600 bg-indigo-50 border border-indigo-100/50 px-2 py-0.5 rounded-full whitespace-nowrap">
                        {addon.price}
                      </span>
                    </div>
              ...
              <p className="mt-4 text-[10px] text-slate-400 font-medium text-center">
                Add-ons are billed monthly alongside your plan. Cancel any add-on anytime.
              </p>
            </div>
          </div>
        )}
          ...
          {showUpgradeModal && (
            <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.95 }}
                className="bg-white rounded-3xl p-6 max-w-sm w-full border border-slate-200 shadow-2xl space-y-5 text-center"
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white mx-auto shadow-lg">
                  <Lock size={16} />
                </div>
                <h3 className="text-base font-extrabold text-slate-900">{upgradeFeature || 'Upgrade to access'}</h3>
                <p className="text-xs text-slate-500 font-medium leading-relaxed">
                  This feature is available on <span className="font-extrabold text-indigo-600">Starter</span> plan (₹399/mo) and above.
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    onClick={() => {
                      handleConfirmUpgrade('starter');
                    }}
                    className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-extrabold transition-all shadow-sm cursor-pointer"
                  >
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h3 className="text-sm font-extrabold text-slate-800">Connect Gmail Account</h3>
                      <p className="text-[10px] text-slate-400 font-semibold">1-Click Google OAuth Sync</p>
                    </div>
                  </div>
                  <button
                ...
                <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 text-xs text-slate-600 leading-relaxed font-medium">
                  <p className="font-bold text-slate-800">Why connect your Gmail?</p>
                  <ul className="space-y-1.5 list-disc pl-4 text-[11px]">
                    <li>Incoming customer emails automatically sync to your Unified Inbox.</li>
                    <li>Replies you send from OnlyPage deliver directly from your registered Gmail address.</li>
                    <li>Zero complex SMTP/IMAP passwords required.</li>
                  </ul>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider">Registered Gmail Address</label>
                  <input
                    type="email"
                    value={gmailEmail}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (error) {
                          ...
                          setGmailConnected(true);
                          localStorage.setItem(`onlypage_gmail_connected_${site.id}`, 'true');
                          localStorage.setItem(`onlypage_gmail_email_${site.id}`, gmailEmail);
                          showToast(`Connected Gmail account: ${gmailEmail}`);
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold flex items-center justify-center gap-2 cursor-pointer shadow-sm shadow-rose-100 disabled:opacity-50"
                  >
                    {connectingGmail ? <Loader2 size={14} className="animate-spin" /> : <Mail size={14} />}
                    <span>{connectingGmail ? 'Authorizing...' : 'Authorize with Google'}</span>
                  </button>
                </div>
              </motion.div>
            </div>
        ...
        {/* Client-Side Zero-Cost QR Code Modal */}
        <QRModal
